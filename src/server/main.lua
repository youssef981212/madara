...
KashChosen = function(charId)
  ...
  KashPlayers[identifier] = {src = _source, id = xPlayer.PlayerData.license}
end
...
PurchaseShop = function(source,callback,shop)
  ...
  if can_buy then
    local identifier = (KashCharacters[source] and KashCharacters[source]..":" or '')..xPlayer.PlayerData.license
...
GetVehicleOwner = function(source,callback,plate)
  SqlFetch('SELECT * FROM player_vehicles WHERE plate=@plate',{['@plate'] = plate},function(data)
    local owner = false
    if data and type(data) == 'table' and data[1] and type(data[1]) == 'table' and (data[1].plate == plate or plate:match(data[1].plate)) then
      print(data[1].license)
      owner = data[1].license
    end
    callback(owner)
  end)
end
...
VehiclePurchased = function(shopId,vehId,props)
  ...
  if Customs.VehicleModels[props.model] ~= nil then
    vehiclemodel = Customs.VehicleModels[props.model].model
  else
    -- Handle the case where the vehicle model is not found.
  end
  ...
  SqlExecute("INSERT INTO player_vehicles SET license=@owner,citizenid = '"..xplauyerc.PlayerData.citizenid.."' ,plate=@plate,mods=@vehicle,vehicle = '"..(vehiclemodel or '').."', hash = '"..props.model.."'",{['@owner'] = VehicleShops[shopId].owner,['@plate'] = props.plate,['@vehicle'] = json.encode(props)})
end
...
TryBuy = function(source, callback, shop, veh, plate, class)
  ...
  if can_purchase then
    ...
    local vehiclemodel       = Customs.VehicleModels[plate.model].model
    ...
    SqlExecute("INSERT INTO player_vehicles SET license=@owner,citizenid = '" .. xPlayer.PlayerData.citizenid .. "',vehicle = '" .. vehiclemodel .. "',hash = '" .. plate.model .. "',plate=@plate,mods=@vehicle,state = 1", {
...
DriveVehicle = function(source,callback,shop,veh)
  local vehData = CopyTable(VehicleShops[shop].stock[veh])
  local vehiclemodel = Customs.VehicleModels[vehData.vehicle.model].model
  for k,v in pairs(VehicleShops[shop].stock) do
    if v.vehicle.plate == vehData.vehicle.plate then
      table.remove(VehicleShops[shop].stock,k)
      break
...
local xplayer = QBCore.Functions.GetPlayer(source)
  SqlExecute("INSERT INTO player_vehicles SET license=@owner ,citizenid = '"..xplayer.PlayerData.citizenid.."',plate=@plate,vehicle = '"..vehiclemodel.."',hash = '"..vehData.vehicle.model.."',mods=@vehicle,state = 1",{['@owner'] = VehicleShops[shop].owner,['@plate'] = vehData.vehicle.plate,['@vehicle'] = json.encode(vehData.vehicle)})
  TriggerClientEvent("VehicleShops:Sync",-1,VehicleShops)
  callback(true)
end
